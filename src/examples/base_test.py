...
def setup():
    ...
    # nodes
    setup.add_node([0.0, 0.0, 0.0], mass=0.01, fixed=True)
    setup.add_node([1.0, 1.0, 0.0], mass=0.01, fixed=False)
    setup.add_node([0.0, 1.0, 0.0], mass=0.01, fixed=False)
    setup.add_node([1.0, 0.0, 0.0], mass=0.01, fixed=False)

    # bars
    setup.add_bar(0, 1, stiffness=10.0, rest_length=2 ** 0.5, damping=0.1)
    setup.add_bar(0, 2, stiffness=10.0, rest_length=1.0, damping=0.1)
    setup.add_bar(2, 1, stiffness=10.0, rest_length=1.0, damping=0.1)
    setup.add_bar(1, 3, stiffness=10.0, rest_length=1.0, damping=0.1)
