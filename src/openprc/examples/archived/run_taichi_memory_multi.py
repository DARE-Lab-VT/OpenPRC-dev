...
# --- Configuration ---
TRIAL_NAME = "Taichi_IID_Memory_Opt_30Hz_multi_k"
ROWS, COLS = 4, 4
...
class AugmentedLagrangian:
    def __init__(self, n_elements, kappa=0.0, tau=1.0, c_star=0.0):
        self.kappa = kappa
        self.tau = tau
        self.c_star = c_star
        self.prev_violation = 1.0
        self.n = n_elements

        self.sum_field = ti.field(dtype=ti.f64, shape=(), needs_grad=True)
        self.violation_field = ti.field(dtype=ti.f64, shape=(), needs_grad=True)
        
        self.al_kappa = ti.field(dtype=ti.f64, shape=())
        self.al_tau = ti.field(dtype=ti.f64, shape=())
        self.al_cstar = ti.field(dtype=ti.f64, shape=())
        self.update_device_params()

    def update_device_params(self):
        self.al_kappa[None] = self.kappa
        self.al_tau[None] = self.tau
        self.al_cstar[None] = self.c_star

    @ti.kernel
    def reset(self):
        self.sum_field[None] = 0.0
        self.violation_field[None] = 0.0

    @ti.kernel
    def finalize_stats(self):
        m = self.sum_field[None] / float(self.n)
        v = m - self.al_cstar[None]
        self.violation_field[None] = v 

    @ti.func
    def get_loss(self):
        v = self.violation_field[None]
        return self.al_kappa[None] * v + 0.5 * self.al_tau[None] * v * v

    def step_params(self, verbose=True, name="AL"):
        curr_viol = self.violation_field[None]
        self.kappa += self.tau * curr_viol
        
        if curr_viol > self.prev_violation:
            self.tau *= 1.01
        elif curr_viol < self.prev_violation:
            self.tau *= 0.99
            
        self.prev_violation = curr_viol
        if verbose:
            print(f"[{name}] viol={curr_viol:.4f} kappa={self.kappa:.3f} tau={self.tau:.3f}")
        return curr_viol

@ti.data_oriented
class OrthogonalRegularization(AugmentedLagrangian):
    def __init__(self, n_elements, idx_a, idx_b, kappa=0.0, tau=1.0, c_star=0.0, epsilon=1e-4):
        super().__init__(n_elements, kappa=kappa, tau=tau, c_star=c_star)
        self.idx_a = idx_a
        self.idx_b = idx_b
    ...
    @ti.kernel
    def compute_stats(self, probs: ti.template()):
        for i in range(self.n):
            p_a = probs[i, self.idx_a]
            p_b = probs[i, self.idx_b]
            
            numerator = p_a * p_b
            denominator = (p_a + p_b)/2.0 + self.epsilon[None]
            
            normalized_overlap = numerator / denominator
            ti.atomic_add(self.sum_field[None], normalized_overlap)
# ==========================================
...
class ReservoirOptimizationFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, z_star_tensor, pos_tensor, res, u_sim, v_sim, downsample, regs):
        ctx.res, ctx.u_sim, ctx.v_sim = res, u_sim, v_sim
        ctx.downsample = downsample
        ctx.regs = regs

        res.x_rest.from_torch(pos_tensor)
        res.z_star.from_torch(z_star_tensor)
        ...
        disp_30Hz = res.node_disp.to_torch()[::downsample, :, :]
    ...
    def backward(ctx, grad_outputs_30Hz):
        ...
        grad_sim = torch.zeros((res.batch_steps, res.num_mass * 2), dtype=torch.float64, device=grad_outputs_30Hz.device)
        grad_sim[::downsample, :] = grad_outputs_30Hz
        grad_sim_reshaped = grad_sim.reshape(res.batch_steps, res.num_mass, 2)

        res.x_rest.grad.fill(0)
        res.z_star.grad.fill(0)
        res.node_disp.grad.from_torch(grad_sim_reshaped)
        ...
        with ti.ad.Tape(loss=res.loss, validation=False):
            ...
            for reg in regs:
                reg.reset()
                reg.compute_stats(res.z_star)
                reg.finalize_stats()
                add_reg_loss(res, reg)

        grad_pos = res.x_rest.grad.to_torch()
        fixed_mask = torch.tensor(res.fixed_mask.to_numpy(), device=grad_pos.device)
        grad_pos = grad_pos * (1.0 - fixed_mask.unsqueeze(1))
        grad_z_star = res.z_star.grad.to_torch()
        return grad_z_star, grad_pos, None, None, None, None, None
...
if __name__ == "__main__":
    ...
    z_param = nn.Parameter(torch.zeros((len(config['springs']), M_LEVELS), dtype=torch.float64))
    ...
    for i in range(M_LEVELS):
        for j in range(i+1, M_LEVELS):
            regs.append(OrthogonalRegularization(len(config['springs']), i, j, tau=1.0))
    ...
    for epoch in range(EPOCHS):
        ...
        X_30Hz = ReservoirOptimizationFunction.apply(z_star_tensor, pos_param, res_opt, u_sim, v_sim, DOWNSAMPLE, regs)
        ...
        loss = torch.sum(mse_per_task)

        loss.backward()
        torch.nn.utils.clip_grad_norm_([z_param], max_norm=1.0)
        optimizer.step()
        for reg in regs:
            reg.step_params(name=f"AL({reg.idx_a},{reg.idx_b})", verbose=True)
            reg.update_device_params()

        max_viol = max([reg.violation_field[None] for reg in regs]) if regs else 0.0
