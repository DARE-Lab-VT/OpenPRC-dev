...
def generate_openprc_grid_config(rows=4, cols=4):
    """Generates Horizontal/Vertical springs with 3-Node Hinges (No overlapping diagonals)."""
    SPACING = 0.053     
    STIFFNESS = 222.15  
    DAMPING = 0.8
    NODE_MASS = 0.01    
    ...
    # 2. Generate Horizontal & Vertical Springs Only
    springs_list = []
    for r in range(rows):
        for c in range(cols - 1):
            springs_list.append([node_indices[r, c], node_indices[r, c + 1], SPACING, STIFFNESS, 0.0, DAMPING, 0.0])
    for r in range(rows - 1):
        for c in range(cols):
            springs_list.append([node_indices[r, c], node_indices[r + 1, c], SPACING, STIFFNESS, 0.0, DAMPING, 0.0])
    
    # 3. Generate 90-Degree Hinges
    hinges_list = []
    HINGE_STIFFNESS = 0.005 
    for r in range(rows - 1):
        for c in range(cols - 1):
            n_tl = node_indices[r, c]
            n_tr = node_indices[r, c + 1]
            n_bl = node_indices[r + 1, c]
            # Hinge: [Node 1, Vertex, Node 2, Torque_K, Rest_Angle]
            hinges_list.append([n_tr, n_tl, n_bl, HINGE_STIFFNESS, np.pi/2])
...
class DifferentiableReservoir:
    def __init__(self, config, dt, batch_steps):
        ...
        self.hinge_k_tor = ti.field(dtype=ti.f64, shape=self.num_hinges)
        self.hinge_rest_angle = ti.field(dtype=ti.f64, shape=self.num_hinges)
    ...
    def _initialize_from_config(self, config):
        ...
        for h in range(self.num_hinges):
            ...
            self.hinge_k_tor[h], self.hinge_rest_angle[h] = hinges[h, 3], hinges[h, 4]
    ...
    def compute_step(self, t: int, u_val: ti.f64, v_val: ti.f64):
        ...
        for s in range(self.num_springs):
            ...
            # FIX 1: Move epsilon inside square root for fully differentiable norm
        ...
        # 3. Rotational Forces (Torsion)
        for h in range(self.num_hinges):
            i, j, k = self.hinge_i[h], self.hinge_j[h], self.hinge_k[h]
            v_ji = self.x[t, i] - self.x[t, j]
            v_jk = self.x[t, k] - self.x[t, j]
            
            l_ji_sq, l_jk_sq = v_ji.norm_sqr() + 1e-12, v_jk.norm_sqr() + 1e-12
            cross_vec = v_ji.cross(v_jk)
            
            # FIX 1.5: Differentiable magnitude for cross product
            cross_mag = ti.math.sqrt(cross_vec.norm_sqr() + 1e-12)
            
            current_angle = ti.math.atan2(cross_mag, v_ji.dot(v_jk))
            torque = -self.hinge_k_tor[h] * (current_angle - self.hinge_rest_angle[h])
            n_dir = cross_vec / cross_mag
            
            F_i = (torque / l_ji_sq) * n_dir.cross(v_ji)
            F_k = -(torque / l_jk_sq) * n_dir.cross(v_jk)
            F_j = -F_i - F_k
            
            ti.atomic_add(self.force_buffer[i], F_i)
            ti.atomic_add(self.force_buffer[j], F_j)
