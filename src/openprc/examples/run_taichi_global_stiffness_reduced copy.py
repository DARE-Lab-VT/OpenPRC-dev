...
# --- Configuration ---
RUN = "M2_NL"
TRIAL_NAME = f"Taichi_subspring_global_stiffness/{RUN}"
ROWS, COLS = 4, 4
    ...
    def step_params(self, viols, verbose=False):
        with torch.no_grad():
            for (i, j), viol_tensor in viols.items():
                ...
                if viol_val > self.prev_viol[i, j] * 0.9:
                    self.tau[i, j] *= 1.01
...
if __name__ == "__main__":
    ...
    # L[C] (Eq. 13, Dambre 2012): 30 degree-1 targets at delays 0–29; uniform MSE loss.
    # NL[C] (Eq. 14, Dambre 2012): all ~46k targets (tau_d=29, n=4); degree-weighted MSE loss.
    task_degree_weights = None
    if LOSS_MODE == "linear_memory":
        y_target_raw = generate_benchmark_target_matrix(u_30Hz, tau_d=29, k=1, n=1)
    elif LOSS_MODE == "nonlinearity":
        y_target_raw, task_exps = generate_benchmark_target_matrix(
            u_30Hz, tau_d=29, k=1, n=4, return_exps=True)
        task_degrees_np = task_exps.sum(axis=1).astype(np.float64)
        task_degree_weights = torch.tensor(
            task_degrees_np / task_degrees_np.sum(), dtype=torch.float64, device=device)
    else:
        raise ValueError(f"Unknown LOSS_MODE: {LOSS_MODE!r}. Use 'linear_memory' or 'nonlinearity'.")
    print(f"[{LOSS_MODE}] Target matrix shape: {y_target_raw.shape}")
    ...
    for epoch in range(EPOCHS):
        ...
        if task_degree_weights is None:
            loss = torch.mean(mse_per_task)
        else:
            loss = torch.sum(task_degree_weights * mse_per_task)
