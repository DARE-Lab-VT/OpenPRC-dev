...
# --- Configuration ---
TRIAL_NAME = "Taichi_IID_Legendre_Opt"
ROWS, COLS = 4, 4
...
def legendre_P_normalized(n, x):
    """Recursively computes Normalized Legendre Polynomials exactly like OpenPRC."""
    if n == 0: return np.ones_like(x)
    pm2, pm1 = np.ones_like(x), x.copy()
    for k in range(1, n):
...
def generate_benchmark_target_matrix(u_input, tau_d, k, n, tau_step=1):
    ...
    # 1. Scale u exactly to [-1, 1] to ensure Legendre Polynomials are truly orthogonal
    u_min, u_max = np.min(u), np.max(u)
    if abs(u_max - u_min) < 1e-12:
        raise ValueError("Input signal is constant. Cannot scale to [-1,1].")
    u_leg = 2.0 * (u - u_min) / (u_max - u_min) - 1.0

    # 2. Window viewing over the scaled Legendre input
    U_sub = sliding_window_view(u_leg, max_lag + 1)[:, ::-1][:, [j * k for j in range(0, tau_d + 1, tau_step)]]
    Lvars = U_sub.shape[1]
    ...
    # 3. Multiply Normalized Legendre values instead of raw exponents
    for j in range(Lvars):
        unique_degrees = np.unique(exps[:, j])
        unique_degrees = unique_degrees[unique_degrees > 0]
        if len(unique_degrees) == 0: continue
        
        # Precompute Legendre over this lag to save time
        leg_cache = {deg: legendre_P_normalized(deg, U_sub[:, j]) for deg in unique_degrees}
        
        for idx_exp, deg in enumerate(exps[:, j]):
            if deg > 0:
                Y_valid[:, idx_exp] *= leg_cache[deg]
...
if __name__ == "__main__":
    ...
    # [CRITICAL FIX 3]: OpenPRC Feature drops the reference node explicitly.
    ...
    for epoch in range(EPOCHS):
        ...
        # --- [RIDGE TRACE FIX START] ---
        # 100% faithful to the imitation.py solver formulation
        XTX = X_train.T @ X_train
        p1 = X_train.shape[1]
        lam = 1e-6 * (torch.trace(XTX) / p1)
        
        I = torch.eye(p1, device=X_train.device) * lam
        I[0, 0] = 0.0 # Mathematically crucial: DO NOT regularize the bias term
        
        W_out = torch.linalg.solve(XTX + I, X_train.T @ y_train_torch)
        # --- [RIDGE TRACE FIX END] ---
        
        # MSE and Capacities (R2 = 1 - MSE since Var(y)=1)
        mse_per_task = torch.mean((X_train @ W_out - y_train_torch)**2, dim=0)
        r2_per_task = 1.0 - mse_per_task
        
        # IPC Capacity respects the Dambre threshold.
        valid_capacity = torch.where(r2_per_task > dambre_eps, r2_per_task, 0.0)
        total_capacity = torch.sum(valid_capacity).item()
        
        # Loss uses Softmax weighting to heavily penalize high MSE tasks 
        # (forcing the physics to generate difficult memory lags)
        ...
        print(f"Epoch {epoch+1:03d} | Softmax MSE: {loss.item():.5f} | IPC Capacity: {total_capacity:.2f} | Max MSE: {mse_per_task.max().item():.5f}")
