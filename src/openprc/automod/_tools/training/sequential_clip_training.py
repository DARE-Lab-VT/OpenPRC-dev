"""
Sequential cumulative PRC readout training study.

This script trains cumulative readouts by adding trajectory clips in their
natural order, for example:

  corridor_000
  corridor_000 + corridor_001
  corridor_000 + corridor_001 + corridor_002
  ...

It does not modify train_readout.py. It creates temporary filtered bundle
manifests and launches train_readout.py as a subprocess.
...
def _plot_curves(
    ...
    for feature in features:
        for target in targets:
            ...
            ax.set_title(f"Sequential Clip Curve: {target} / {feature}")
            ...
            _savefig(
                fig,
                plot_dir / f"sequential_curve_{_safe_token(target)}_{_safe_token(feature)}.png",
                also_pdf,
            )
            plt.close(fig)
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Train cumulative readouts by adding clips in sequence."
    )
    ...
    out_dir = args.output_dir or (
        args.bundle_dir / args.robot / "training" / "sequential_clip_training"
    )
    ...
    print("=== Sequential clip training study ===")
    ...
    csv_path = out_dir / "sequential_learning_curve.csv"
    summary_path = out_dir / "sequential_summary.json"
    ...
    print("\n=== Sequential clip training report complete ===")
