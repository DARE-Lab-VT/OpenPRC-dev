...
Go1 PRC Pipeline
================
Full Physical Reservoir Computing pipeline for the Unitree Go1 quadruped:

  Stage 1 — Reservoir preprocessing:
    Walk the Go1's URDF and turn each link's visual mesh into a spring-mass
    reservoir spec (.npz) stored under go1/reservoir/.
  ...
  Stage 2 — Batch simulation:
    ...
    Output lands in go1/reservoir_sims/<trajectory_id>/.
  ...
  Stage 3 — Readout training:
    ...
    run grouped/temporal k-fold CV to pick ridge regularization lambda,
    refit on the full train split, and evaluate on the held-out test split.

  Stage 4 — Plots:
    Time-series, scatter, residual histograms, and rollout-decay curves.

Usage:
    python examples/pipeline_go1_prc.py --bundle-dir /path/to/robot_bundle

Assumptions:
    - The bundle already has Go1 trajectories ingested
      (run _tools/fetch/fetch_go1.py first if needed).
    - The manifest.json exists (run automod.build_manifest if not).
    - CUDA is available for Stage 2; fall back to --backend cpu if not.
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Go1 PRC pipeline")
